...
vector<string> splitStringAndbuildVector(string &str, int TAG)//TAG=0(Other),1(Month),2(Day_Of_Week);
{
    str += ",";
    vector<string> vret;
    ...
    while(found != string::npos)
    {
        ...
        if(fdx == string::npos)//非连续值
        {
            if(TAG == 1 && isalpha(x[0]))
                x = to_string_x(mMon[x]);//是month英文缩写，转换为数字
            if(TAG == 2 && isalpha(x[0]))
                x = to_string_x(mWek[x]);//是day of week英文缩写，转换为数字
            if(x.size() == 1)
                x = "0" + x;//添加0
            vret.push_back(x);
        }
...
int main()
{
    ...
    while(n--)
    {
        ...
        set<string> wekexist;
        for(size_t i = 0; i < vdfw.size(); ++i)
            wekexist.insert(vdfw[i]);//更快的检索当前日期（dayofweek）是不是应该执行
        int curyear = syInt;//从开始年份执行
        while(curyear <= eyInt)
        {
            if(isleapyear(curyear))
                mtharray[2] = 29;//leapyear的2月份应该是29天
            else
                mtharray[2] = 28;
            ...
            for(size_t mi = 0; mi < vmth.size(); mi++)//month
            {
                ...
                for(size_t di = 0; di < vdfm.size(); di++)//day of month
                {
                    ...
                    if(wekexist.count(wd) == 0 || stoi_x(curd) > mtharray[stoi_x(curm)])
                        continue;
                    ...
                    for(size_t Hi = 0; Hi < vhur.size(); ++Hi)//hour
                    {
                        for(size_t Mi = 0; Mi < vmts.size(); ++Mi)//minutes
                        {
                            ...
                            if(datetime >= st && datetime < et)
                                mrt[datetime].push_back(command);//在当前日期时间内
                        }
                    }
                }
            }
