...
struct Authority{
    ...
    int level;
    Authority() {}
    Authority(string &_category, int _level = WITHOUT_LEV):category(_category), level(_level) {};
    bool operator<= (Authority _auth) {
        return _auth.category == category && level <= _auth.level;
    }
};

struct Role {
    vector<Authority> authority_vector;
    string name;
    Role() {}
    Role(vector<Authority> &_authority_vector, string &_name): authority_vector(_authority_vector), name(_name) {};
    int inquire(Authority _auth) {
        for (int i = 0; i < authority_vector.size(); i++) {
            if (_auth <= authority_vector[i]) {
                if (_auth.level == WITHOUT_LEV && authority_vector[i].level != WITHOUT_LEV) return authority_vector[i].level;
                else return TRUE;
            }
        }
        return FALSE;
...
struct User {
    ...
    string name;
    User() {}
    User(vector<Role> &_role_vector, string &_name):role_vector(_role_vector), name(_name) {};
    int inquire(Authority _auth) {
        int MAX = FALSE;
        for (int i = 0; i < role_vector.size(); i++) {
            Role role = role_vector[i];
            int temp = role.inquire(_auth);
            MAX = (temp > MAX) ? temp : MAX;
        }
        if (MAX == TRUE)    return TRUE;
        else if (MAX == FALSE)  return FALSE;
        else    return MAX;
    }
};

int string2int(string &_str) {
    stringstream ss;
    ss << _str;
    int re;
    ss >> re;
...
Authority makeAuthority(string _str) {
    int pos = (int)_str.find(':');
    if (pos == -1) {
        return Authority(_str);
    }
    string categroy = _str.substr(0, pos);
    string lev = _str.substr(pos + 1, 1);
    int level = string2int(lev);
    return Authority(categroy, level);
}

map<string, Authority> authority;               //保存权限
map<string, Role> roles;                        //保存角色
map<string, User> users;                        //保存用户
...
void run() {
    ...
    for (int i = 0; i < p; i++) {
        cin >> input;
        Authority temp = makeAuthority(input);
        authority.insert(make_pair(temp.category, temp));
    }
    cin >> r;
    for (int i = 0; i < r; i++) {
        string name;
        ...
        int n;
        cin >> n;
        vector<Authority> temp_vec;
        for (int j = 0; j < n; j++) {
            cin >> input;
            Authority temp = makeAuthority(input);
            temp_vec.push_back(temp);
        }
        Role role = Role(temp_vec, name);
        roles.insert(make_pair(name, role));
    }
    cin >> u;
    for (int i = 0; i < u; i++) {
        string name;
        ...
        int n;
        cin >> n;
        vector<Role> temp_vec;
        for (int j = 0; j < n; j++) {
            cin >> input;
            Role role = roles[input];
            temp_vec.push_back(role);
        }
        User user = User(temp_vec, name);
    ...
    for (int i = 0; i < q; i++) {
        string name, auth;
        cin >> name >> auth;
        Authority author = makeAuthority(auth);
        User user = users[name];
        int res = user.inquire(author);
        if (res == TRUE)    cout << "true" << endl;
        else if (res == FALSE)  cout << "false" << endl;
        else    cout << res << endl;
    }
}
